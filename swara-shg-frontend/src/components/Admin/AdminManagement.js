...
const AdminManagement = () => {
  ...
  const getRoleBadge = (role) => {
    if (role === 'staff') {
      return (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: '#fff8e1', color: '#e65100', padding: '3px 10px', borderRadius: '10px', fontSize: '11px', fontWeight: 700 }}>
          <Eye size={12} /> Staff
        </span>
      );
    }
    return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: '#e8eaf6', color: '#283593', padding: '3px 10px', borderRadius: '10px', fontSize: '11px', fontWeight: 700 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f9fa' }}>
      ...
      <div style={{ padding: '24px', fontFamily: 'sans-serif', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <h1 style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '6px 0 0', fontSize: '24px' }}>
              <KeyRound size={22} /> Admin Management
            </h1>
            <p style={{ margin: '4px 0 0', color: '#666' }}>Manage admin accounts for Swara SHG</p>
          </div>
          <button
            onClick={() => setShowCreateModal(true)}
            style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '10px 20px', background: '#1a1a2e', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '14px' }}
          >
        ...
        {/* Info banners */}
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', background: '#e8f5e9', border: '1px solid #a5d6a7', borderRadius: '8px', padding: '12px 16px', marginBottom: '12px', fontSize: '13px', color: '#2e7d32' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px', marginBottom: '24px' }}>
          {[
            ...
            <div key={c.label} style={{ background: c.bg, borderRadius: '12px', padding: '16px', borderLeft: `4px solid ${c.color}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', color: '#666', marginBottom: '4px', fontWeight: 600, textTransform: 'uppercase' }}>
                <c.Icon size={13} color={c.color} /> {c.label}
              </div>
              ...
              <thead>
                <tr style={{ background: '#1a1a2e' }}>
                  {['#', 'Name', 'Email', 'Role', 'Created', 'Status', 'Actions'].map(h => (
                    <th key={h} style={{ padding: '14px 16px', color: 'white', textAlign: 'left', fontWeight: 600 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={admin.id} style={{ borderBottom: '1px solid #f0f0f0', background: idx % 2 === 0 ? 'white' : '#fafafa' }}>
                      ...
                      <td style={{ padding: '14px 16px' }}>
                        <div style={{ fontWeight: 700, display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          {isMe && (
                            <span style={{ background: '#e3f2fd', color: '#1565c0', padding: '2px 8px', borderRadius: '10px', fontSize: '11px', fontWeight: 600 }}>
                              You
                            </span>
                          )}
                        </div>
                      ...
                      <td style={{ padding: '14px 16px' }}>
                        {admin.isActive
                          ? (
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: '#e8f5e9', color: '#2e7d32', padding: '4px 12px', borderRadius: '12px', fontSize: '12px', fontWeight: 700 }}>
                              <CheckCircle2 size={13} /> Active
                            </span>
                          ) : (
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: '#ffebee', color: '#c62828', padding: '4px 12px', borderRadius: '12px', fontSize: '12px', fontWeight: 700 }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          {!isMe && (
                            <button
                              ...
                              style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '5px 12px', background: '#e3f2fd', color: '#1565c0', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}
                            >
                          ...
                          {!isMe && (
                            <button
                              ...
                              style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '5px 12px', background: admin.isActive ? '#fff8e1' : '#e8f5e9', color: admin.isActive ? '#e65100' : '#2e7d32', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}
                            >
                              {admin.isActive
                          ...
                          {!isMe && (
                            <button
                              ...
                              style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '5px 12px', background: '#ffebee', color: '#c62828', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}
                            >
      ...
      {showCreateModal && (
        ...
        >
          <div
            style={{ background: 'white', borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '480px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}
              ...
              {createForm.password && (
                <div style={{ marginTop: '8px' }}>
                  {[
                    ...
                    <span key={r.label} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px', marginRight: '10px', color: r.test ? '#2e7d32' : '#bbb', fontWeight: 600 }}>
      ...
      {showResetModal && selectedAdmin && (
        ...
        >
          <div
            style={{ background: 'white', borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '440px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}
            onClick={e => e.stopPropagation()}
          >
            <h2 style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '0 0 4px', color: '#1a1a2e', fontSize: '20px' }}>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', position: 'fixed', bottom: '24px', right: '24px', zIndex: 9999, padding: '14px 20px', borderRadius: '8px', fontWeight: 600, fontSize: '14px', boxShadow: '0 4px 16px rgba(0,0,0,0.18)', background: toast.type === 'error' ? '#c62828' : '#2e7d32', color: 'white' }}>
