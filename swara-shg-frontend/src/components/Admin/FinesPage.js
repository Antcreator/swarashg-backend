...
const FinesPage = () => {
  ...
  const [stats, setStats]         = useState({ savingsFineTotal: 0, chamaaFineTotal: 0, unpaidFinesCount: 0 });
      ...
      <div className="admin-container">
        ...
        <div className="page-header">
          <div>
            <Link
              to="/admin/dashboard"
              style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#1976d2', textDecoration: 'none', fontSize: '14px' }}
            >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button
              ...
              style={{
                ...
                alignItems: 'center', gap: 5, fontSize: '13px', color: '#555',
              }}
            >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          <div style={{ background: '#fff3e0', border: '2px solid #ff9800', borderRadius: '10px', padding: '18px' }}>
            <p style={{ margin: '0 0 4px', color: '#555', fontSize: '13px' }}>Savings Fines Total</p>
          ...
          <div style={{ background: '#fce4ec', border: '2px solid #e91e63', borderRadius: '10px', padding: '18px' }}>
            <p style={{ margin: '0 0 4px', color: '#555', fontSize: '13px' }}>Chamaa Fines Total</p>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
          {[
            ...
            { key: 'savings_late', label: `Savings Fines (${fines.filter(f => f.fineType === 'savings_late').length})` },
            { key: 'chamaa_late',  label: `Chamaa Fines (${fines.filter(f => f.fineType === 'chamaa_late').length})` },
          ].map(({ key, label }) => (
            <button key={key} onClick={() => setActiveTab(key)} className={`btn-${activeTab === key ? 'primary' : 'secondary'}`}>
              {label}
            </button>
          ))}
        </div>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr
                    key={f.id}
                    style={{
                      background: f.isPaid ? 'rgba(76,175,80,0.06)' : undefined,
                  ...
                  >
                    ...
                    <td>
                      <span style={{
                        ...
                        background: f.fineType === 'savings_late' ? '#fff3e0' : '#fce4ec',
                        color: f.fineType === 'savings_late' ? '#e65100' : '#880e4f',
                        border: `1px solid ${f.fineType === 'savings_late' ? '#ff9800' : '#e91e63'}`,
                        display: 'inline-flex', alignItems: 'center', gap: 5,
                      }}>
                        {f.fineType === 'savings_late'
                          ? <><PiggyBank size={11} /> Savings</>
                          : <><RefreshCw size={11} /> Chamaa</>}
                      </span>
                    </td>
